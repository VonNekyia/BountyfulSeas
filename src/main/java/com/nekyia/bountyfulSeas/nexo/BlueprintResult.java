 ...
 */
public record BlueprintResult(Path target, List<String> created, String note) {

    public BlueprintResult {
        created = List.copyOf(created);
    }

    static BlueprintResult written(Path target, List<String> created) {
        return new BlueprintResult(target, created, null);
    }

    static BlueprintResult unchanged(Path target) {
        return new BlueprintResult(target, List.of(), null);
    }

    static BlueprintResult skipped(String note) {
        return new BlueprintResult(null, List.of(), note);
    }
